...
import DespiaDemo from '../pages/DespiaDemo.jsx';

function App() {
  const [theme, setTheme] = useState(
    window.location.search.includes('theme=material') ? 'material' : 'ios'
  );
  const [currentColorTheme, setCurrentColorTheme] = useState('k-color-brand-yellow');
  ...
  // Set yellow color theme by default
  useLayoutEffect(() => {
    const htmlEl = document.documentElement;
    htmlEl.classList.forEach((c) => {
      if (c.includes('k-color')) htmlEl.classList.remove(c);
    });
    htmlEl.classList.add('k-color-brand-yellow');
  }, []);
